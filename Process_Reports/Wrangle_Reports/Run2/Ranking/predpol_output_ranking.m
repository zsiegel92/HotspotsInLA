%RUN from the Predpol Project/Code directory
% load('driving_crimes_by_division.mat');
% load('binClarity.mat');
% binClarity = binClarity';
folderNames = {'rankings','reported_only','rankings/rankedBIN_NUMBERS','rankings/ranked_with_names'};
...
for i = 1:21
    ...
    %WRITING TO FILE
        filename3 = ['Process_Reports/Wrangle_Reports/Run2/Ranking/reported_only/predpol_predicted_LACITY_',divNames{i},'_reportedbins.csv'];
    ...
    for j = 1:size(rankings,2)
        for k = 1:size(rankings,1)
            if (rankings(k,j)>0)
                rankingsNames{k,j}=strrep(binClarity{rankings(k,j),6},',',';'); %column 5 is div number, column 6 is address
            else
    ...
    filename4 = ['Process_Reports/Wrangle_Reports/Run2/Ranking/rankings/ranked_with_names/predpol_predicted_LACITY_',divNames{i},'_rankedBins_locNames.csv'];
    ...
    formatspec1 = [formatspec1(1:end-1),'\n'];
    filename5 = ['Process_Reports/Wrangle_Reports/Run2/Ranking/rankings/rankedBIN_NUMBERS/predpol_predicted_LACITY_',divNames{i},'_rankedBins.csv'];
