...
descent_codes = 'BWHO';
avgDists = cell(21,1);
totAvgDists = cell(21,1);

for l = 1:21
    avgDists{l} = zeros(length(descent_codes),length(kValues));
    totAvgDists{l} = zeros(1,length(kValues));
    
    descents = specified_arrests_by_division_race{l}(:,9);
    descents = [descents{:}]';
    for j = 1:length(descent_codes)
        arrests_by_race = find(descents==descent_codes(j));
        avgDists{l}(j,:) = mean(closestRealDist{l}(arrests_by_race,:)); %mean of each column where row is for given descent code
    end
    totAvgDists{l}(1,:)=mean(closestRealDist{l}(:,:)); %mean of each column
end
...
for l = 1:21
    ...
    tot_counter(l) = size(closestRealDist{l},1);
end
clear descs;
...
filename = 'externalStatsRun2/2015_Distance_from_Hotspots_byRace_withStats.csv';
...
for l = 1:21
    ...
    lineFront = ['Total (',num2str(tot_counter(l)),' arrests)'];
    line = [lineFront,num2cell(totAvgDists{l}(1,printKValues))]; %print total
    fprintf(fid,linespec,line{1,:});
